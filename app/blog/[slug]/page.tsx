...
const mockPosts = [
  {
    ...
    content: "<p>الأعمال المدنية هي الأساس الذي تُبنى عليه جميع المشاريع الإنشائية، حيث تلعب دوراً حاسماً في ضمان استقرار المباني وسلامتها عبر الزمن.</p><p>نحن في DGR Diamond Growth نؤمن بأن جودة الأعمال المدنية تعني جودة المشروع بأكمله، لذلك نستخدم أفضل التقنيات والمواد لتحقيق أعلى المعايير الممكنة.</p>",
    featuredImage: "/service-civil.jpg",
    publishedAt: "2024-01-15T10:00:00Z",
    author: "شركة DGR Diamond Growth",
  ...
  },
  {
    ...
    content: "<p>في ظل التحول الرقمي السريع، أصبحت شبكات الاتصالات من أهم عناصر البنية التحتية للمدن الذكية والمؤسسات الحديثة.</p><p>توفر شبكات الألياف الضوئية سرعات نقل بيانات هائلة، مما يعزز من قدرة المؤسسات على أداء أعمالها بكفاءة وموثوقية عالية، وهذا ما نحرص على تقديمه في DGR Diamond Growth.</p>",
...
export async function generateMetadata({ params }: { params: { slug: string } }) {
  ...
  const metaTitle = `${post.title} - مدونة DGR Diamond Growth`
...
export default function BlogPostPage({ params }: { params: { slug: string } }) {
  ...
  const schema = {
    ...
    headline: title,
    image: image,
    author: { "@type": "Person", name: "شركة DGR Diamond Growth" },
    publisher: {
      "@type": "Organization",
      name: "شركة DGR Diamond Growth",
  ...
  return (
    <main className="min-h-screen bg-white">
      <Head>
        <title>{title} - مدونة DGR Diamond Growth</title>
